...
The staging proposed; this session disposes. Re-derive the Decision against the bar — it stands only when the fork lives at product level (what the product's user gets or how it behaves, never how the tree achieves it), the costs conflict irreducibly (no measurement, convention, spec entry, or further trace breaks the tie), and the tie-break is the user's — with the context the staging lacked: user rulings, deferrals, ground that has moved — proposals approved earlier in this same walk included. And a side no informed user would choose is not a side: a fork with one live side is settled.
