...
// resolve within the embedded set. No cause is wrapped (none changes what the
// user can do), and the slug named is the fallback's, never the nomination's —
// naming the nomination would send a user to fix their prefs for a fault in the
// binary.
