...
// brokenBuiltinFormat is §14A's PINNED user-facing sentence for the one state
// §7.6 leaves genuinely fatal: a theme a slot falls back to cannot be loaded
// from the embedded set.
//
// It is a single-sourced constant rather than an fmt.Errorf at the return site
// because it is new user-facing copy, and copy is pinned rather than left
// implicit even when — especially when — it sits on a path the build-time
// guarantee makes unreachable. Terse is right here: the binary is broken, the
// user cannot fix it, and there is nothing to advise beyond naming which
// built-in is at fault.
//
...
// BrokenBuiltinError returns §7.6's fatal for a fallback slug that does not
...
// and neither answer changes what the user can do. §8.5's "one not-loadable path
// serves every cause" applies here too, and appending a reason to a sentence
// that already says "this binary is broken" would only make the line longer.
//
