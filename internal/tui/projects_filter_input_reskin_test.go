...
// list. styleFilterInput now restyles BOTH m.sessionList.FilterInput and
// m.projectList.FilterInput through the same shared per-list helper, so the two
// pages can never drift. These assertions mirror the Sessions filter-input gate
// (filtering_reskin_test.go) for the project list: an accent.orange `/ ` prompt,
// accent.orange query text, and an accent.orange block cursor (blink off) in the
// coloured branch (Dark/Light), dropping to a bare NoColor input under the NO_COLOR
// carve-out. Colour roles are pinned in exact mode-resolved SGR (the §2.9 token
// core), like the sibling filtering-reskin tests — a token swap is caught, not
// merely a glyph's presence. No t.Parallel() (the package-level mock convention and
// the shared canvas helpers make parallelism unsafe across this package's tests).
...
// TestProjectsFilterInput_ColourlessBranchBare asserts the §2.5 NO_COLOR carve-out:
// the project FilterInput keeps the `/ ` prompt but emits NO accent.orange SGR — the
// prompt, query, and cursor render on the terminal's native fg, and the cursor is a
// bare NoColor block with blink disabled.
