...
// Adapter opens one new host-terminal window. It quarantines every
// OS/terminal-specific concern so general code switches on Result alone.
...
// Outcome is a closed taxonomy. "Unsupported" is deliberately not a member:
// whether a host terminal has a driver is a resolution-tier decision taken
// before any OpenWindow call.
type Outcome int

const (
	// OutcomeUnknown is the zero value, so a bare Result{} fails OK() instead
	// of passing as a success that would gate a self-attach. An adapter must
	// never return it.
...
// Result's Detail and Guidance are opaque driver-owned payloads — Detail rides
// up only as a log attr, Guidance only on the permission path. General code
// classifies on Outcome.
type Result struct {
	Outcome  Outcome
	Detail   string
	Guidance string
