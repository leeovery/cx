...
// themeCommitFailedEvent is §12.3's message for a failed theme write, emitted
// under the `theme` component — `theme: commit failed` as it reads in the log.
const themeCommitFailedEvent = "commit failed"

// themePersister commits the theme panel's choice (§8.9). It is the production
// tui.ThemePersister, and it is owned by cmd rather than by prefs or by the TUI
// because three constraints meet here — the same three that decided §10.5's
// ownership: prefs is a deliberate leaf that must not import internal/log, the
// write needs prefs path resolution, and the `theme` component records the
// failure.
//
...
// event or make internal/tui a fourth package emitting a component §8.9 closes at
// three.
//
// It logs AND RETURNS. Phase 9's outstanding-failure state machine renders
// `⚠ couldn't save theme` from the returned value and holds the failure
// outstanding (§9.13), so a persister that only logged would recreate the silent
// "applied but not persisted" state the picker idiom exists to close.
//
// The merge itself stays inside prefs, behind the field-specific savers (§8.9):
// this type resolves nothing about the record's contents, holds no snapshot of
// it, and so cannot clobber a sibling field. Each instance persists its own
...
// in the same write (§8.2's mutual exclusion, enforced by the saver).
...
// constant and leaving the other slot untouched (§9.5's `● both` in two
// keypresses).
//
...
// reportCommit emits §12.3's `theme: commit failed` for a failed write and hands
// err back verbatim. A successful commit emits NOTHING: the persister's one event
// is a failure, and the commit-time `theme: loaded` belongs to the loader.
//
// `reason` is appended HERE rather than at either call site, so the attr order —
// §12.3's `slug`, `slot`, `reason` — is one decision in one place rather than two
...
// themeSlotAttr renders a prefs slot as §12.3's `slot` attr value, and reports
...
// assert a distinction §8.2's two-state setting does not have, and an empty one
