...
// themeCmd is the theme verb group. Export is its one member: the picker's theme
// panel covers listing and selecting, while export is the only route from a
// built-in living inside the binary to a file on disk the user can edit.
...
// deliberately never creates or seeds the themes directory, and a redirect will
...
var themeExportCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Export never reads prefs, so it is not covered by the rule that
		// strips a persisted slug on the way in and needs its own site — but the
		// refusal frame echoes the argument back on stderr, and an argument can
		// carry a pasted escape exactly as a prefs value can. Stripping at the
		// read rather than at the echo is what makes the stripped value THE
...
// exportRefusal composes the stderr frame for one export failure.
//
...
// reason string is also all that discriminates them, because the exit code is 1
// for every class; nothing about the failure is scriptable, so nothing is encoded
// numerically.
//
...
// and the file is never opened — and the generic frame is the right rendering if
// that ever ceased to hold, so there is no dead special case.
//
...
func exportRefusal(slug string, rejection *theme.Rejection) error {
	...
	case theme.ReasonUnreadable:
		// Detail is the OS error verbatim — the only thing that
...
//
// The ordering — charset check, THE EMBEDDED SET, then the themes directory — is
// not implemented here. `theme.Loader.ResolveByName` owns it, and export shares it
// with TUI construction, so the two cannot drift about what a slug means. A second
// implementation of an ordering rule is a rule that will eventually be two rules,
// and this one carries the no-shadowing guarantee on a path that never enumerates.
//
// What it returns is the SOURCE, not a re-serialisation of the parsed Theme.
// Re-serialising would drop every `#` comment, and the comments are the
// attribution header the file format was chosen to carry and the eyeball-pin
// derivation notes that are the only surviving record of a judgement nothing else
// can re-derive. The theme is still parsed first: that is what refuses an invalid
...
//
// EVERY failure comes back as a Rejection, never as a bare error, so exportRefusal
...
// LOCATED into an `unreadable` rejection, carrying the resolution error verbatim.
//
...
//
// Surfacing the resolution error raw instead would put an unpinned sentence on a
// surface whose copy is fixed.
