...
4. **The tree answered questions argument could not.** The two-number split, the self-healing hypothesis, the build-tag hazard and the fixed-seed gate's guarantee were each settled — three of them reversed — by measuring rather than reasoning. The `-shuffle=on` probe found a live order-dependency in `cmd` before a line moved, and reading Go's own `M.Run` showed a gate we had already decided proved less than claimed.
