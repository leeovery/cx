# Consolidation Tasks: resume-hooks-silently-lost (Phase 4)

## Task 1: One fake for the pane-token enumeration
placement: phase 4
severity: near-miss

**Problem**: `mockPaneHookLister` (`cmd/hooks_test.go:129-138`, new in 4-3) declares `rows []tmux.PaneHookRow` / `err error` / `calls int` and counts every read. `recordingHookKeyLister` (`cmd/hookkey_vocabulary_test.go:65-82`) already declares exactly that trio (`hookKeyCalls` as the counter) plus `TryGetServerOption`, and it lives in the file whose own header declares itself the home for "the enumeration rows they arrive in, and the seam fakes that answer with them". `*recordingHookKeyLister` already satisfies the new `PaneHookLister` interface structurally.

**Solution**: Site the plain `rows`/`err`/`calls` fake in `cmd/hookkey_vocabulary_test.go` and have `recordingHookKeyLister` embed it, keeping only `TryGetServerOption` and its own `restoring`/`restoringErr` fields.

**Outcome**: One fixture for one seam, in the file that declares itself its home, and the sweep tests and the list tests share it instead of each carrying a copy.

**Do**:
- Move the plain fake into `cmd/hookkey_vocabulary_test.go` and embed it in `recordingHookKeyLister`.
- Rename `hookKeyCalls` → `calls` at its five sweep-test consumers: `cmd/run_hook_stale_cleanup_test.go:268`, `:401`, `:423`, `:450`, `:483`.
- **Keep `loudPaneHookLister` (`cmd/hooks_test.go:313-319`) exactly as it is** — its behaviour is to fail on read, which is not the same fixture, and task 4 gives it more work.
- The `PaneHookLister` interface stays as authored — narrower than `AllPaneLister`, one method. Nothing about the seam changes; only the fixture stops being written twice.
- The pre-existing three-fake merge (`stubAllPaneLister`, `fakeHookLister`) is deliberately **not** in scope.
...
**Tests**:
- Existing: `TestHooksListLocationColumn`'s subtests and the five sweep tests must stay green unchanged
- No new test — a fixture merge earns none

## Task 2: The `hook` verb drivers get one home, and read through the reader the package already has
placement: phase 4
severity: drift

**Problem**: Task 4-3 put `runHookList` in the shared staging home (`cmd/testhelpers_test.go:60-73`); task 4-2 put `runHookRm` (`cmd/hooks_rm_exit_test.go:16-25`), `seedHooksFile` (`:29-37`) and `assertHooksFileUnchanged` (`:39-48`) in its own suite file. All three are staging by `cmd/testhelpers_test.go`'s own definition ("how a test is set up and driven"), and `runHookSet` is already there — so the phase leaves the three `hook` verb drivers under two different rules. Separately, both new helpers hand-roll `os.ReadFile` + `t.Fatalf` where `readFileBytes(t, path)` already exists for the same package at `cmd/bootstrap_production_test.go:129-141`.

**Solution**: Move `runHookRm`, `seedHooksFile` and `assertHooksFileUnchanged` into `cmd/testhelpers_test.go` beside `runHookSet` and `runHookList`, and have both read through the existing `readFileBytes`.

**Outcome**: One home holds all three `hook` verb drivers and their staging, and the package has one file reader rather than three.

**Do**:
- Move the three helpers into `cmd/testhelpers_test.go`.
- Re-point both at `readFileBytes`. **Carry this nuance deliberately**: `readFileBytes` returns nil on ENOENT, so a deleted `hooks.json` would fail the byte comparison instead of fatalling on the read — same verdict, different message. No test changes colour; confirm that rather than assuming it.
- **Guard (a) — do NOT re-point the `hooks …` alias blocks in `cmd/hooks_test.go`** at the shared drivers. They drive the back-compat alias verbatim (`:53`, `:71`, `:119`, `:343`, `:365`, `:391`, `:416`, `:436`, `:443`, `:464`, `:527`, `:549`, `:576`, `:602`, `:628`, `:649`, `:673`, `:702`, `:733`, `:763`, `:796`, `:825`, `:855`, `:880`, `:934`, `:958`) and the drivers drive the canonical `hook`. Re-pointing them silently deletes alias coverage — a hazard a phase-2 bank entry predicted.
- **Guard (b)** — cross-reference the two helper-file headers so a reader landing on either learns where the other half lives.

**Acceptance Criteria**:
- `runHookSet`, `runHookRm` and `runHookList` all live in `cmd/testhelpers_test.go`, as do `seedHooksFile` and `assertHooksFileUnchanged`
- Both helpers read through `readFileBytes`; no new `os.ReadFile` + `t.Fatalf` pair remains in either
- Every `hooks …` alias block still drives the alias, not the canonical verb — alias coverage is unchanged in count and in what it drives
- The two helper-file headers cross-reference each other
- No assertion changes and no test changes its verdict
- `go test ./...` and `go test -tags integration -p 1 ./cmd/...` pass

**Tests**:
- Existing: the whole `cmd` hooks suite must stay green unchanged
- Prove guard (a) held by counting the alias drives before and after — the count and their targets must be identical

## Task 3: The new `readFileBytes` in `internal/hooks` has an un-migrated twin in its own file
placement: phase 4
severity: duplication

**Problem**: `readFileBytes` (`internal/hooks/store_test.go:43-53`, new in 4-1) sits in the same file as an inline `os.ReadFile` + `t.Fatalf` + `bytes.Equal` at `:1170-1176` asserting the same thing — that a failed save left the seeded file byte-identical.

**Solution**: Replace the inline read at `:1170-1176` with `readFileBytes(t, seeded)`, leaving the comparison and its message as they are.

**Outcome**: The helper the phase introduced is used by every site in its own file that wants it.

**Do**:
- Replace the inline read at `internal/hooks/store_test.go:1170-1176`. Leave the comparison and failure message untouched.
- **Not in scope**: the same helper would subsume four more copies at `internal/hooks/store_shape_test.go:33-38`, `:55-60`, `:110-115`, `:130-135`. Those are pre-existing and recorded as debt; do not fold them in.

**Acceptance Criteria**:
- The inline read at `:1170-1176` is gone, replaced by `readFileBytes`
...
**Tests**:
- Existing: the failed-save subtest must stay green with its assertion and message unchanged

## Task 4: The tests that Execute `hook list` inject the seam it now reaches
placement: phase 4
severity: drift

**Problem**: After 4-3 the list body reaches tmux — `cmd/hooks.go:147` calls `buildPaneHookLister()`, which falls back to `tmux.DefaultClient()`. Five sites Execute that body with no `PaneLister` injected: `cmd/hooks_test.go:53`, `:71`, `:934`, and `cmd/root_test.go:299` and `:302` (whose table injects `hooksDeps` only for the `set`/`rm` rows). All five stay off tmux only because their store is empty and the `len(list) == 0` guard at `cmd/hooks.go:143` returns before the read — an implicit dependency no assertion states. CLAUDE.md is explicit: a test that Executes a real command body MUST inject every tmux-touching `*Deps` seam, and `cmd`'s `TestMain` TMUX poison is the structural enforcement that would otherwise catch a missed injection loudly.

**Solution**: Inject `hooksDeps = &HooksDeps{PaneLister: &loudPaneHookLister{t: t}}` (with the usual `t.Cleanup`) at the five sites.

**Outcome**: Today's incidental isolation becomes the stated one, and a future edit that removes the empty-list guard fails loudly at these sites instead of reaching for a real tmux server.

**Do**:
- Inject at `cmd/hooks_test.go:45-63` (`:53`), `:65-81` (`:71`), `:925-946` (`:934`), and `cmd/root_test.go:299` and `:302`.
- Use the existing `loudPaneHookLister` (`cmd/hooks_test.go:313-319`) — 4-3 wrote it for exactly this contract, so no new vocabulary appears.
- `cmd/root_test.go`'s table currently injects `hooksDeps` only for the `set`/`rm` rows; extend it to the list rows in the same shape rather than inventing a second mechanism.
- Every site's store is empty, so no assertion changes.

**Acceptance Criteria**:
- All five sites inject a `PaneLister`
- The fake used is the existing `loudPaneHookLister`; no new fake is introduced
- No assertion changes and no test changes its verdict
- The injection is real, not decorative: removing the `len(list) == 0` early return from `cmd/hooks.go:143` makes these sites fail loudly rather than reach `tmux.DefaultClient()`
- `go test ./...` and `go test -tags integration -p 1 ./cmd/...` pass

**Tests**:
- Existing: all five sites stay green unchanged
- Prove the injection is load-bearing by mutation — delete the empty-list early return and confirm the five sites fail on the loud fake rather than dialling a real server. Revert.

## Task 5: The docs describe the pane-token enumeration as stale-cleanup's alone
placement: phase 4
severity: comments

**Problem**: `CLAUDE.md:60` calls `ListAllPaneHookKeys` "the stale-cleanup `list-panes -a -F` enumeration …", and `CLAUDE.md:166` repeats "stale-cleanup enumerates live tokens via `tmux.ListAllPaneHookKeys`". As of 4-3 there is a second consumer — `paneLocationsByToken` (`cmd/hooks.go:160-184`), reached through the new `PaneHookLister` seam — and the `Location` half is now rendered to the user as `hook list`'s fourth column. Separately `README.md:201`'s `hook list` line still reads "list all hooks" and never mentions that column.

**Solution**: Name the enumeration by what it returns rather than by one caller, record `hook list`'s live read and its fourth column in the Resume-hook command row, and correct the README line.

**Outcome**: Nothing in the docs describes the enumeration as single-consumer, and the column `hook list` now prints is documented where a user would look for it.

**Do**:
- Correct `CLAUDE.md:60` so the enumeration is named by what it returns, not by one of its two callers.
- In the same file's Resume-hook command row, note that `hook list` resolves each entry's location through **one** live enumeration per invocation, rendered as a fourth tab-separated column that is empty when no live pane carries the token, when the key is not token-shaped, or when the read fails (no server running).
- Correct `README.md:201`'s `hook list` line to mention the fourth column. **This one is an orchestrator call**: the finder classified it plan-authorable rather than consolidation, and I am folding it in — it is one stale doc sentence about the exact change the rest of this task documents, and splitting it into separate work would be ceremony. Say so if you disagree.
- Documentation only. No code moves, nothing changes colour.
- **Do not touch** the phase-3 bank entry's subject — CLAUDE.md's missing description of the shape-aware reaper and `session.IsTokenShaped`. That is a different sentence and phase 5's paragraph.
...
**Tests**:
- None. A documentation edit changes no behaviour; the suites passing unchanged is the whole proof.

## Bank Disposition

- Pre-existing repo-wide lint/format debt outside this task's surface. — residue — pre-existing repo-wide lint/format debt; no phase-4 file involved
...
- The restore-window rule is now single-sourced in cmd, but the daemon states the inverse error policy twice with no shared name. — residue — outside phase 4 surface
...
- A third reader of the same failed-read-counts-as-set posture sits in cmd, in a file the daemon entry does not name. — residue — outside phase 4 surface
...
- TestDoctorFixPrunedHookOutput is now fully subsumed by a sibling test. — residue — outside phase 4 surface
...
- Four further cmd test files touching doctor.go / run_hook_stale_cleanup.go still carry concern-derived names. — residue — outside phase 4 surface
...
- staleSeed is now declared twice, byte-identical, in one file. — residue — outside phase 4 surface
...
- The removal leaves two byte-identical subtests in TestDoctorStaleHooksCheck. — residue — outside phase 4 surface
...
- The gone-pane message reaching the user is three wraps deep, and Phase 4 reworks the same call site for hook rm wording. — residue — phase 4 reworked the call site but trimming a wrap changes user-visible text — fails the no-behaviour-change bar
...
- A saver-hosting integration fixture omits the teardown guard CLAUDE.md prescribes for exactly its shape. — residue — outside phase 4 surface
...
- stubAllPaneLister is the last piece of the hook-sweep seam vocabulary still sited with a single consumer. — residue — outside phase 4 surface
...
- Package cmd now holds two helper-only test files that want to be one. — folded into task 1
...
- Two package-level names in package cmd now hold one key value. — residue — outside phase 4 surface
...
- A few hook-file preamble sites outside the three hooks test files could take the bare helper now that it exists in the shared file. — residue — outside phase 4 surface
...
- The two-helper-file rule is stated in only one of the two files, and phase 4 could silently collapse alias coverage. — folded into task 2
...
- The last hand-rolled level filter now lives in internal/spawn, and it is not the same filter. — residue — outside phase 4 surface
...
- The same five-property audit-breadcrumb block is still written inline outside this task two helpers. — residue — outside phase 4 surface
...
- Two near-duplicate TMUX_PANE-unset subtests in TestHooksSetCommand, one a strict subset of the other. — residue — outside phase 4 surface
...
- assertHookFireCount reads the hook-fire file with no wait, racing the hydrate helper across the whole restore hook-fire family. — residue — outside phase 4 surface
...
- Unit-lane real-tmux restore tests silently depend on whichever portal is installed; root cause is a bare portal in the hydrate argv. — residue — outside phase 4 surface
...
- CLAUDE.md never describes the shape-aware reaper or internal/session new token surfaces, so the retain-old-format-forever rule is undocumented. — residue — outside phase 4 surface
...
- Four integration tests exec portal state hydrate from the ambient PATH instead of staging a binary — residue — root-cause fix changes the production hydrate argv and re-tags a suite; plan-authorable, surfaced for decision
...
- Four fixture helpers in restore_test are pairwise generalisations of one another — residue — same root cause as the hydrate-argv entry; plan-authorable, surfaced for decision
...
- Four hand-rolled (component,msg) exactly-one-record sink filters remain across three packages; internal/logtest is the natural home — residue — outside phase 4 surface
...
- The marker bracket set half is unobserved by every caller, not just internal/restore — residue — outside phase 4 surface
...
- Two more copies of the scrollback seeder survive in cmd/bootstrap, in the one package that actually asserts on the payload — residue — outside phase 4 surface
...
- readHookKey in internal/tmux is a third fixture pane-token read that must NOT be consolidated — residue — outside phase 4 surface
...
- A third production bare -t issuer exists in the daemon capture loop, exposed to the rename class this workflow is about — residue — outside phase 4 surface
...
- project.Store.Remove still carries the identical defect task 4-1 just removed from hooks — residue — identical defect in a different store, outside this work unit scope — needs a scoping decision, not a phase-4 task
...
- The new readFileBytes helper duplicates an inline read-and-compare left by an earlier task in this unit — folded into task 3
...
- Byte-identity file assertions are hand-rolled at eight-plus sites across the cmd test suite, and 4-2 adds a ninth — folded into task 2
...
- cmd/hooks.go now carries four near-identical nil-check dependency builders — residue — daemon capture loop, outside phase 4 surface — needs a scoping decision alongside the project-store entry
