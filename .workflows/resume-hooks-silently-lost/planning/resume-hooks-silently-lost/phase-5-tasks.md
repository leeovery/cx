...
**Do**:
- Add `internal/hooks/lock.go` holding: the exported sentinel `ErrLockHeld` (on the precedent of `state.ErrDaemonLockHeld`, so a caller can `errors.Is` a timeout apart from a save failure); package-level `lockTimeout = 2 * time.Second` and a short poll interval, both `var` so the unit lane can lower them; `func (s *Store) lockPath() string` returning `s.path + ".lock"`; and unexported `acquireLock(path string, openFlags int, flockMode int) (*os.File, error)`, which opens `path` with the caller's flags and polls `unix.Flock(fd, flockMode|unix.LOCK_NB)` until it succeeds or `lockTimeout` elapses, returning a wrapped `ErrLockHeld` on expiry and closing the fd on every failure path. `flockMode` and `openFlags` are parameters because task 5-2's shared read path passes `unix.LOCK_SH` and opens without `O_CREAT`.
...
- Add a test-only bound seam beside the var — `func SetLockTimeoutForTest(t *testing.T, d time.Duration)`, taking `*testing.T` first and restoring the prior value through `t.Cleanup` — on the shape of `log.SetTestHandler`, so `cmd`-level tests in later tasks can drive a timeout without waiting out the production figure.
...
- The bound and the poll interval are package-level `var`s the unit lane lowers through `SetLockTimeoutForTest`; no test asserts the production figure as a timing measurement
...
**Do**:
- In `internal/hooks/lock.go` add the shared acquire: the same `acquireLock` with `openFlags` of `os.O_RDONLY` — **no `O_CREAT`** — and `flockMode` of `unix.LOCK_SH`. Carry one line of comment on the omitted `O_CREAT`, wording left to the executor.
...
- Re-point the three production read call sites: `cmd/run_hook_stale_cleanup.go`'s advisory pre-read passes `"internal"`, `cmd/doctor.go`'s `checkStaleHooks` passes `"doctor"`, and `cmd/hooks.go`'s `hooksListCmd` passes `"cli"`. Leave the non-locking `load()` inside a mutation's exclusive hold emitting nothing — it is not degraded, it is already exclusive.
...
- [ ] A read while the sidecar is held exclusively elsewhere returns its data anyway, after the bound, rather than failing
...
- [ ] The cycle returns at the bound rather than waiting longer, so the daemon's 1s tick is not parked, and there is no retry loop, backoff or escalation inside the cycle
...
- `"it returns at the bound"` — assert the cycle's elapsed time is within a small multiple of the lowered bound
...
- The daemon's 1s tick must not be parked, so the cycle returns at the bound rather than waiting longer
