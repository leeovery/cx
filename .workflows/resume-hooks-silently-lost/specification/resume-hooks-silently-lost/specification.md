...
The degraded read is the one genuinely new emission: DEBUG, `op=load-unlocked`, the lock error in `error`, and `via` naming the caller — `hydrate` for `LookupOnResume`, `doctor` for `checkStaleHooks`, the existing `cli` for `hook list`, and the existing `internal` for the sweep's advisory pre-read (§6.3), which degrades by the same rule and adds no value of its own. That adds **two `op` values** — `load-unlocked` here and `touch-save-requested` for the dirty-flag touch (§2.2) — **two `via` values, one component binding in `cmd` (§2.2) and no attr key**: the whole of this work unit's amendment to the closed logging vocabulary.
