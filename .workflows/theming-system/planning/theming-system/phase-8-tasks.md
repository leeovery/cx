...
> Phase 5 task 5-4 declared `SlotResolution{Slot, Requested, Resolved, FellBack, Reason, Theme}` with `Requested` documented as "the slug that was nominated (shipped default when the slot was unset)" — which is precisely the pre-fallback value this table needs, and why the derivation reads the resolution record rather than the nomination.
...
**Outcome**: `go run ./cmd/capturetool --fixture theme-panel-adaptive-pair --theme nord`, with the fixture's `captureKeys` `t` typed (by the tape, or by hand in the live view — `capturetool` replays no keys of its own), renders the panel over the Sessions list with `● light` / `● dark` badges on two rows; `--fixture theme-panel-constant-previewing --theme tokyo-night-day` under the same `t` renders a bare `●` on `nord` while the cursor sits on a different row — the picker idiom made visible — and both frames reach no real themes directory and no prefs.
