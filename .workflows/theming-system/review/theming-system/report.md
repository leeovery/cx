...
Beneath that, 488 non-blocking notes — triaged rather than worked through, for a reason the distribution makes plain: 488 over 171 tasks, mean 2.85, median 3, and only 4 tasks producing none. Defects clump; that is flat, which makes the count a function of how many verifiers ran rather than of how much is wrong. 140 are applied, 75 declined as comment prose the current standard would not have asked for, 253 marked won't-fix as test-hygiene and speculative tail, and 2 dropped with the CHANGELOG entry they maintained. **18 stay open**: 5 spec corrections, 4 decisions and 9 bugs, none blocking.
...
- **Defects hiding in the quick-fix bucket** — a slice-aliasing `append` writing into its own input; an empty `Requested` able to claim a badge under the empty identity; six assertions that could not fail; dead code with no caller; and six guards or probes that did not catch what they claimed.
...
The tail, after every note naming a concrete defect or a real decision was taken out. Two kinds:

- **Test hygiene (191)** — shared-helper extractions, redundant subtests, fixture consolidation, parameter order, naming. ~85% in `_test.go`, none touching behaviour or what the suite proves. Applying them rotates ~28,000 lines of test code for no change in coverage against a real risk of breaking working tests.
- **Speculative ideas (62)** — overwhelmingly "consider whether…" and "decide whether…": memoising a render pass, making recorded figures self-checking, whether a positional bool reads opaquely. They propose no change anyone is owed.

Both are symptoms of one verifier per task, each asked to review as a senior architect and each reliably producing two or three observations. If the duplication is ever worth addressing it is worth one deliberate consolidation pass, not 253 separate edits. Recorded by target so nothing is lost; the per-task reports carry each note in full.
...
*5 notes. Four amend other work units' completed specs, which the historical-artifact correction protocol gates on explicit confirmation; the fifth is this feature's own §9.1.*
...
16. `workflows/portal-observability-layer/specification/portal-observability-layer/specification.md`:173 — `.workflows/portal-observability-layer/specification/portal-observability-layer/specification.md:173` — the `bootstrap` owns-row reads "The 11-step bootstrap orchestrator"; as-built the orchestrator is ten steps (CLAUDE.md:126; former step 11 `CleanStale` was re-homed onto the `_portal-saver` daemon). Line 869 repeats "the 11-step bootstrap orchestrator". Change both to "ten-step". This stale number sits in the very table this amendment edited and, unlike the `clean` row, was not surfaced anywhere — so a file that now carries a corrigendum block asserting its counts are reconciled still serves one wrong count to the knowledge base. (Report 10-9)
