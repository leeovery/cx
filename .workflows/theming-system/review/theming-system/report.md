...
The two blocking items were both incomplete sweeps rather than defects, and both are now closed. Task 10-10's sign-off capture clear ran, and then six panel geometry/state tapes and their PNGs were re-created for the phase 13–15 visual gates and never re-cleared — so the tree ships scaffolding that three documents written by this same feature (`testdata/vhs/README.md`, `CLAUDE.md`, `internal/capture/fixtures.go:61`) each assert is absent. Task 14-2 swept retired token names out of comments but not out of the `t.Errorf`/`t.Fatalf` strings its own Do list placed in scope, leaving 151 dead token names across 37 test files — and files where the rewritten comment and the failure message two lines below it now name different tokens. Neither blocks a user; both leave the repo asserting something false about itself, which is the specific failure mode this feature spent two phases guarding against elsewhere.

Beneath that, 488 non-blocking notes — triaged rather than worked through, for a reason the distribution makes plain: 488 over 171 tasks, mean 2.85, median 3, and only 4 tasks producing none. Defects clump; that is flat, which makes the count a function of how many verifiers ran rather than of how much is wrong. 152 are applied, 75 declined as comment prose the current standard would not have asked for, 257 marked won't-fix, 2 dropped with the CHANGELOG entry they maintained. **2 stay open** — one condition, needing a specification decision rather than a fix.
...
Both are **resolved** in this review pass (`eaeedd4c`).

1. ~~Clear the residual capture scaffolding from `testdata/vhs/`~~ — **done.** Six tape/PNG pairs from the phase 13–15 panel-geometry gates had survived task 10-5's sign-off clear. `README.md`, `LOCK-IN.md` and `reference/` are kept; all 27 fixture builders are untouched, so §13.4's swap-and-diff guard enumerates exactly what it did before. (Report 10-10)

2. ~~Finish task 14-2's sweep into test failure strings~~ — **done.** 149 retired token names swept from failure messages and role-label table data across 35 files. The two surviving `border.footer` references are `active_theme_test.go`'s absence guard, which exists to prove the retired shade renders nowhere. `TestNoRetiredTokenNameInComments` is now `TestNoRetiredTokenName` and walks string literals as well as comments; verified non-vacuous by poisoning a literal. (Report 14-2)

## Findings disposition

**Applied 152** · **Declined 75** · **Won't fix 257** · **Dropped 2** · **Open 2**
...
1. `internal/theme/resolve.go` — 2 notes, one condition
...
Spec §5.6 states the by-name path "looks for `<slug>.theme` and nothing else", which is what makes a duplicate slug structurally impossible, and §5.7 keeps that path free of a directory read. Reconciling the two surfaces touches that reasoning rather than only the code, so it is left as a specification decision.
